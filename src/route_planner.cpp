...
void RoutePlanner::AddNeighbors(RouteModel::Node *current_node) {
    ...
    for (RouteModel::Node* neigh: (*current_node).neighbors){
        ...
        neigh->h_value = this->CalculateHValue(neigh);
        neigh->g_value = current_node->g_value + current_node->distance(*neigh);
        this->open_list.push_back(neigh);
        neigh->visited = true;
    }
}
...
void RoutePlanner::AStarSearch() {
    ...
    RouteModel::Node *next = this->NextNode();
    while (next != end_node){
        this->AddNeighbors(next);
        next = this->NextNode();
    }
    std::vector<RouteModel::Node> final_path = this->ConstructFinalPath(next);
